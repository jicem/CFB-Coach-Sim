...
func _ready():
	...
	for i in database.query_result:
		...
		database.update_rows("seasons", "sid = " + str(Global.season + 1), data)
		
		var streak = 0
		var school
		var school_array = database.select_rows("teams", "tid = " + str(team), ['school'])
		...
		if streak > 14:
			database.update_rows("achievements", "gid = 3", {"achieved": 1})
		elif streak > 6:
			database.update_rows("achievements", "gid = 2", {"achieved": 1})
		elif streak > 2:
			database.update_rows("achievements", "gid = 1", {"achieved": 1})
			
		database.update_rows("seasons", "sid = " + str(Global.season + 1), {"streak": streak})
