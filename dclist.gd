...
func _ready():
	...
	var array2 : Array = database.select_rows("defcoordinators", "", ["*"])
	for row in array2:
		var scheme
		# Create variable for tree row
		treerow = tree.create_item()
		# Convert jersey number to string
		var textID = str(row["dcid"])
	...
	if selection.text != "":
		...
		if id > 0 and id < 81:
			...
			for row in array2:
				...
				if newBudget >= 0:
					# Update dcid and budget values in the new teams table for the player's team
					database.update_rows("teams1", "tid == " + str(team), {"dcid": id})
